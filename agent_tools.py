...
class GenerateGridLayout(BaseTool):
    ...
    parameters = [{
        ...
        'description': 'Type of grid: "hex" or "rect". Default "hex".', 'required': False
    }, {
        'name': 'radius', 'type': 'number',
        'description': 'Aperture radius in meters. Default 0.13.', 'required': False
    }, {
        'name': 'output_path', 'type': 'string',
        'description': f'Optional save path. Default: {DEFAULT_LAYOUT_PATH}', 'required': False
    }]
...
class ApplyVortexPhase(BaseTool):
    ...
    parameters = [{
        'name': 'charge_l', 'type': 'integer',
        'description': 'Topological charge.', 'required': True
    }, {
        'name': 'file_path', 'type': 'string',
        'description': 'Optional path to CSV.', 'required': False
    }]
...
class ApplyDammannGrating(BaseTool):
    ...
    parameters = [
        {'name': 'file_path', 'type': 'string', 'description': 'Optional CSV path.', 'required': False},
        {'name': 'beam_order', 'type': 'integer', 'description': 'Beam order N. Supports [2-7].', 'required': True},
        {'name': 'period_multiple', 'type': 'integer', 'description': 'Grating period in unit cells. Default 12.', 'required': False}
    ]
    ...
    def call(self, params: str, **kwargs) -> str:
        try:
            ...
            raw_transitions = {
                2: [0.5], 3: [0.2656], 4: [0.1767, 0.4577],
                5: [0.1322, 0.4808], 6: [0.1084, 0.2941, 0.4519],
...
class ApplyCollimateLens(BaseTool):
    description = 'Apply collimating lens phase (theoretical). phi = k*(sqrt(x^2+y^2+f^2) - f).'
    parameters = [
        {'name': 'focal_length', 'type': 'number', 'description': 'Focal distance (m). Default 0.15.', 'required': False},
        {'name': 'mode', 'type': 'string', 'description': '"overwrite" or "add". Default "overwrite".', 'required': False},
        {'name': 'file_path', 'type': 'string', 'description': 'Optional CSV path.', 'required': False}
    ]
...
class ApplyAxiconPhase(BaseTool):
    ...
    parameters = [
        {'name': 'cone_angle_deg', 'type': 'number', 'description': 'Half-angle (deg). Default 10.', 'required': False},
        {'name': 'mode', 'type': 'string', 'description': '"overwrite" or "add".', 'required': False},
        {'name': 'file_path', 'type': 'string', 'description': 'Optional CSV path.', 'required': False}
    ]
...
class ApplyAiryPhase(BaseTool):
    ...
    parameters = [
        {'name': 'cubic_coeff', 'type': 'number', 'description': 'Cubic coefficient. Default 5e6.', 'required': False},
        {'name': 'separable', 'type': 'boolean', 'description': 'Separable 2D. Default true.', 'required': False},
        {'name': 'mode', 'type': 'string', 'description': '"overwrite" or "add".', 'required': False},
        {'name': 'file_path', 'type': 'string', 'description': 'Optional CSV path.', 'required': False}
    ]
...
class ApplyBeamSteering(BaseTool):
    ...
    parameters = [
        {'name': 'steer_theta_deg', 'type': 'number', 'description': 'Elevation angle (deg). Default 30.', 'required': False},
        {'name': 'steer_phi_deg', 'type': 'number', 'description': 'Azimuth angle (deg). Default 0.', 'required': False},
        {'name': 'mode', 'type': 'string', 'description': '"overwrite" or "add".', 'required': False},
        {'name': 'file_path', 'type': 'string', 'description': 'Optional CSV path.', 'required': False}
    ]
    ...
    def call(self, params: str, **kwargs) -> str:
        try:
            ...
            x, y = df['x'].values, df['y'].values
            steer_phase = k * (x * np.sin(theta) * np.cos(phi) + y * np.sin(theta) * np.sin(phi))
            if 'target_phase' not in df.columns:
                df['target_phase'] = 0.0
            if mode == 'overwrite':
...
class ConfigureCPEfficiency(BaseTool):
    description = 'Configure circular polarization conversion efficiency and handedness.'
    parameters = [
        {'name': 'eta', 'type': 'number', 'description': 'Conversion efficiency 0~1. Default 0.85.', 'required': False},
        {'name': 'incident_pol', 'type': 'string', 'description': '"LCP" or "RCP". Default "LCP".', 'required': False},
        {'name': 'file_path', 'type': 'string', 'description': 'Optional CSV path.', 'required': False}
    ]
...
class ApplyMeasuredCompensation(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        try:
            ...
            measured_phase = interpolate_to_hex_grid(x_coords, y_coords, phase_2d, hex_x, hex_y)
...
class CalculatePBRotation(BaseTool):
    ...
    parameters = [{'name': 'file_path', 'type': 'string', 'description': 'Optional CSV path.', 'required': False}]
    ...
    def call(self, params: str, **kwargs) -> str:
        try:
            ...
            for ax_idx, (data_arr, title, cmap_name, vmin, vmax) in enumerate([
                (phase_data, f'Compensation Phase ({pol_label})', 'jet', 0, 2*np.pi),
                ...
                for idx, (x, y) in enumerate(xy):
                    ...
                    poly = RegularPolygon((x, y), numVertices=6, radius=radius_hex, orientation=0)
                    patches.append(poly)
                    colors_list.append(c)
                pc = PatchCollection(patches, facecolors=colors_list, edgecolors='grey', linewidths=0.1)
            ...
            for i in range(0, len(X), batch_size):
                ...
                E_cross += np.sum(m_b * amp_cross * np.exp(1j * (p_b + path_diff)), axis=0)
            ...
            def _plot_3d(ax, E_data, title, cmap_name='jet'):
                ...
                surf = ax.plot_surface(Xp, Yp, Zp, cmap=cmap_name, rcount=80, ccount=80, alpha=0.9)
